...
function getExitCodeFromError(error3) {
  ...
  if (isContainerExitNonZeroError(error3)) {
    return +error3.message.toLowerCase().slice(error3.message.toLowerCase().indexOf(UNEXPECTED_EDIT_ERROR) + UNEXPECTED_EDIT_ERROR.length + 1);
  }
  return null;
}
...
var Container = class extends DurableObject {
  ...
  async schedule(when, callback, payload) {
    ...
    if (when instanceof Date) {
      const timestamp2 = Math.floor(when.getTime() / 1e3);
      this.sql`
        INSERT OR REPLACE INTO container_schedules (id, callback, payload, type, time)
        VALUES (${id}, ${callback}, ${JSON.stringify(payload)}, 'scheduled', ${timestamp2})
      `;
      await this.scheduleNextAlarm();
      return {
        taskId: id,
   ...
   * @private
   */
  async scheduleNextAlarm(ms2 = 1e3) {
    const existingAlarm = await this.ctx.storage.getAlarm();
    const nextTime = ms2 + Date.now();
...
var files = pgTable("files", {
  ...
  originalKey: varchar("original_key", { length: 255 }).notNull(),
  fileType: varchar("file_type", { length: 50 }).notNull(),
  mimeType: varchar("mime_type", { length: 100 }).notNull(),
  fileSize: bigint2("file_size", { mode: "number" }).notNull(),
  ...
  hlsPlaylistKey: varchar("hls_playlist_key", { length: 255 }),
  metadata: jsonb("metadata").default({}),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull()
}, (table3) => ({
  userIdIdx: index("files_user_id_idx").on(table3.userId),
  uploadStatusIdx: index("files_upload_status_idx").on(table3.uploadStatus),
  processingStatusIdx: index("files_processing_status_idx").on(table3.processingStatus),
  createdAtIdx: index("files_created_at_idx").on(table3.createdAt)
}));
var processingJobs = pgTable("processing_jobs", {
  ...
  fileIdIdx: index("processing_jobs_file_id_idx").on(table3.fileId),
  statusIdx: index("processing_jobs_status_idx").on(table3.status),
  jobTypeIdx: index("processing_jobs_job_type_idx").on(table3.jobType),
  createdAtIdx: index("processing_jobs_created_at_idx").on(table3.createdAt)
}));
var filesRelations = relations(files, ({ many }) => ({
  processingJobs: many(processingJobs)
}));
var processingJobsRelations = relations(processingJobs, ({ one }) => ({
  file: one(files, {
...
function createDb(databaseUrl) {
  const pool = new Mn({ connectionString: databaseUrl });
...
// .wrangler/tmp/bundle-KBGRgH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KBGRgH/middleware-loader.entry.ts
