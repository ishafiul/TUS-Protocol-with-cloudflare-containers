...
export default function tusDelete(app: HonoApp) {
  app.openapi(
    createRoute({
      method: "delete",
      path: "/files/attachments/{id}",
      tags: ["TUS Upload"],
      description: "Delete an upload session and associated data",
      request: {
        params: z.object({
          id: z.string().openapi({
      ...
      responses: {
        ...
        409: {
          description: "Conflict - upload in progress",
          content: {
            "application/json": {
              schema: z.object({
                error: z.string().openapi({
                  description: "Error message",
                  example: "Cannot delete upload session while upload is in progress"
                })
              })
            }
          }
    ...
    async (c) => {
      ...
      try {
        ...
        console.log({ fileId, context: 'tusDelete', error });
