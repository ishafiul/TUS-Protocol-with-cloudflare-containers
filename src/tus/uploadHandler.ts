...
import {
  DEFAULT_RETRY_PARAMS, isR2ChecksumError, isR2MultipartDoesNotExistError,
  RetryBucket,
...
export const MAX_UPLOAD_LENGTH_BYTES = 1024 * 1024 * 5000
...
export class UploadHandler {
  ...
  constructor(state: DurableObjectState, env: Env) {
    ...
    this.router.options('/api/files/:bucket', optionsHandler)
    this.router.post('/api/files/:bucket', this.exclusive(this.create))
    this.router.options('/api/files/:bucket/:id{.+}', optionsHandler)
    this.router.patch('/api/files/:bucket/:id{.+}', this.exclusive(this.patch))
    this.router.get('/api/files/:bucket/:id{.+}', this.exclusive(this.head))
    ...
    for await (const part of generateParts(body, mem)) {
      ...
      switch (part.kind) {
        case 'intermediate': {
          ...
          const writePart = this.state.storage.put(this.parts.length.toString(), newPart)
        ...
        case 'error': {
          ...
          if (!finished) {
            ...
            console.log('📦 Single-part upload path:', { r2Key, bytesLength: part.bytes.byteLength, finished });
  ...
  async r2Put(r2Key: string, bytes: Uint8Array, checksum?: Uint8Array) {
    ...
    try {
      ...
      console.error('❌ r2Put error:', { r2Key, error: e, message: (e as Error)?.message });
  ...
  async cleanup(r2Key?: string): Promise<void> {
    ...
    try {
      ...
      if (r2Key != null) {
        ...
        // Send file to queue for processing if it's a processable file type
        await this.sendFileToQueue(r2Key)
      }
    }
    catch (e) {
  ...
  private async sendFileToQueue(r2Key: string): Promise<void> {
    try {
      // Get file metadata from storage
      const uploadInfo: StoredUploadInfo & { filename?: string, filetype?: string } = 
        await this.state.storage.get(UPLOAD_INFO_KEY) || {}
      
      if (!uploadInfo.filename || !uploadInfo.uploadLength) {
        console.log('⚠️ Missing file metadata, skipping queue processing')
        return
      }

      const fileInfo: FileInfo = {
        filename: uploadInfo.filename,
        filetype: uploadInfo.filetype || 'application/octet-stream',
        size: uploadInfo.uploadLength,
        r2Key: r2Key,
      ...
      // Send to queue
      const queueService = new QueueService(this.env.FILE_PROCESSING_QUEUE)
      await queueService.sendFileForProcessing(fileInfo, category)
      
    } catch (error) {
      console.error(`❌ Failed to send file to queue: ${r2Key}`, error)
      // Don't throw - we don't want to fail the upload if queue fails
    }
  }
